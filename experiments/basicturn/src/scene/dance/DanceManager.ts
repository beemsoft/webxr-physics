...
export default class DanceManager {
  ...
  constructor(bodyManager: BodyManager) {
    ...
    this.leftFootPosition = new Vec3(this.bodyManager.pelvis.position.x -0.2, 0, this.bodyManager.pelvis.position.z);
    this.rightFootPosition = new Vec3(this.bodyManager.pelvis.position.x + 0.2, 0, this.bodyManager.pelvis.position.z);
  }
  ...
  handleBasicTurn() {
    ...
    if (this.upperBodyTurnDegree > 10 && this.upperBodyTurnDegree < 45) {
      if (this.state === DanceStates.LEFT_TURN2) {
        this.rightFootPosition = new Vec3(this.bodyManager.headInitialPosition.x + 0.2, 0, this.bodyManager.headInitialPosition.z);
      }
    }
      ...
      if (angleDegree1 > this.previousAngleDegree1) {
        if (this.bodyManager.upperBody.quaternion.y < 0) {
          if (this.state === DanceStates.BASIS) {
            ...
            this.rightFootPosition = new Vec3(this.bodyManager.headInitialPosition.x, 0, this.bodyManager.headInitialPosition.z + 0.2);
          }
        } else {
          if (this.state === DanceStates.BASIS) {
        ...
        this.leftFootPosition = new Vec3(this.bodyManager.headInitialPosition.x - 0.2, 0, this.bodyManager.headInitialPosition.z);
